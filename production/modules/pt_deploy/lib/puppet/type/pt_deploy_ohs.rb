...
module Puppet
  Type.newtype(:pt_deploy_ohs) do
    include EasyType
    include ::PtDeployUtils::Validations

    @doc = "This type manages the deployment of an OHS component.
      For deploying OHS, an archive file containing the pre-installed
      bits along with the deployment location should be specified.
      ...
      validate do
        if self[:ensure] == :present
          ...
          # make sure the JDK location is specified
          if self[:jdk_location].nil?
            fail("jdk_location attribute should be specified.")
          end
      ...
      newparam(:jdk_location) do
        desc "The location (directory/folder) where the JDK is deployed."

        validate do |value|
          unless Puppet::Util.absolute_path?(value)
            fail Puppet::Error, "JDK location must be fully qualified, not " + \
                                "'#{value}'"
          end
        end
      end
