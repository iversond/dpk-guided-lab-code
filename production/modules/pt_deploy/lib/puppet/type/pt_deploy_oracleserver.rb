...
module Puppet
  Type.newtype(:pt_deploy_oracleserver) do
    include EasyType
    include ::PtDeployUtils::Validations

    @doc = "This type manages the deployment of an Oracle server component.
      For deploying an Oracle server, an archive file containing the
      pre-installed bits of the Oracle server along with the deployment
      ...
      newproperty(:listener_port) do

        desc "The database listener port."
      ...
      newproperty(:listener_name) do

        desc "The database listener name."
