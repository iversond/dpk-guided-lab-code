...
class pt_profile::pt_cm_postboot_config {
  notify { "Applying pt_profile::pt_cm_postboot_config": }

  $env_type           = hiera('env_type')
  if !($env_type in [ 'fulltier', 'midtier']) {
    fail('The pt_cm_postboot_config profile can only be applied to env_type of fulltier or midtier')
  }
  $pshome_hiera       = hiera('ps_home')
  $pshome_location    = $pshome_hiera['location']
  notify {"POST-Boot Cloud Manager Config PS Home [${pshome_location}]\n":}

    $component_setup_list      = hiera('component_postboot_cm_setup_list')
    $component_setup_order       = hiera('component_postboot_cm_setup_order', '')
    ...
    $component_setup_order_list.each |$component_name| {
      ...
      if ( $component_name == 'cm_file_server') and ($fileserver_creation == false){
        notify {"Cloud Manager POST-Boot File Server creation is disabled":}
      }
      else{
        ...
        notify {"POST-Boot Cloud Manager Component ${component_name} \n":}
