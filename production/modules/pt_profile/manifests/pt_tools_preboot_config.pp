...
class pt_profile::pt_tools_preboot_config {
  notify { "Applying pt_profile::pt_tools_preboot_config": }

  $env_type           = hiera('env_type')
  if !($env_type in [ 'fulltier', 'midtier']) {
    fail('The pt_tools_preboot_config profile can only be applied to env_type of fulltier or midtier')
  }
  $pshome_hiera       = hiera('ps_home')
  $pshome_location    = $pshome_hiera['location']
  notify {"PRE-Boot Tools Config PS Home [${pshome_location}]\n":}

  # check if pre-boot config setup needs to be run
  $run_preboot_config_setup  = hiera('run_preboot_config_setup', true)
  if $run_preboot_config_setup == true {
    ...
    notify {"PRE-Boot setup run is false":}
  }
}
