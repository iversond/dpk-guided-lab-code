 ...
 * ***************************************************************
 */
class pt_role::pt_app_appserver inherits pt_role::pt_base {

  notify { "Applying pt_role::pt_app_appserver": }

  $ensure   = hiera('ensure')
  $env_type = hiera('env_type')

  if $env_type != 'midtier' {
    fail('The pt_app_appserver role can only be applied to env_type of midtier')
  }
