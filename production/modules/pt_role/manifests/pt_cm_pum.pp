 ...
 * ***************************************************************
 */
class pt_role::pt_cm_pum inherits pt_role::pt_app_base {

  notify { "Applying pt_role::pt_cm_pum": }

  $ensure   = hiera('ensure')
  $env_type = hiera('env_type')

  if !($env_type in [ 'fulltier', 'midtier']) {
    fail('The pt_cm_pum profile can only be applied to env_type of fulltier or midtier')
  }

  contain ::pt_profile::pt_cloudmanager_deployment
  contain ::pt_profile::pt_cm_preboot_config
  contain ::pt_profile::pt_cloudmanager  
  contain ::pt_profile::pt_cloudmanager_fileserver

  if $ensure == present {
    contain ::pt_profile::pt_cm_postboot_config
    contain ::pt_profile::pt_password	
    contain ::pt_profile::pt_cloudmanager_patch
