 ...
 * ***************************************************************
 */
class pt_role::pt_fscm_pum inherits pt_role::pt_app_base {

  notify { "Applying pt_role::pt_fscm_pum": }

  $ensure   = hiera('ensure')
  $env_type = hiera('env_type')

  if $env_type != 'fulltier' {
    fail('The pt_fscm_pum role can only be applied to env_type of fulltier')
  }
  if $ensure == present {
