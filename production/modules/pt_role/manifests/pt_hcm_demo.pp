 ...
 * ***************************************************************
 */
class pt_role::pt_hcm_demo inherits pt_role::pt_app_base {

  notify { "Applying pt_role::pt_hcm_demo": }

  $ensure   = hiera('ensure')
  $env_type = hiera('env_type')

  if $env_type != 'fulltier' {
    fail('The pt_hcm_demo role can only be applied to env_type of fulltier')
  }
