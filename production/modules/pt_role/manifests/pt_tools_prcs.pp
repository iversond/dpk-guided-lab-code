 ...
 * ***************************************************************
 */
class pt_role::pt_tools_prcs inherits pt_role::pt_base {

  notify { "Applying pt_role::pt_tools_prcs": }

  $ensure   = hiera('ensure')
  $env_type = hiera('env_type')

  if $env_type != 'midtier' {
    fail('The pt_prcs role can only be applied to env_type of midtier')
  }
